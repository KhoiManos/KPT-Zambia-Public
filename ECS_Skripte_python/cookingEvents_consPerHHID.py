...
def getPaths():
    ...
    return FILE_INPUT, FILE_OUTPUT, FINAL_OUTPUT
...
def getValues():
    ...
    return MIN_CONSUMPTION, MAX_INVALID, TEMP_LIMIT, FUEL_LOOKBACK_MIN
...
def validate_events_optimized(events, df, lookback_minutes, min_consumption):
    ...
    fuel_usage = df[df["consumption (kg)"] > min_consumption][
        ["fuel_id", "timestamp"]
    ].copy()
...
def keep_valid_hhids(cooking_events, max_invalid):
    if "hhid" not in cooking_events.columns:
        cooking_events["hhid"] = (
            cooking_events["exact_id"].astype(str).str.split("_").str[0]
        )
    else:
    ...
    clean_day_set = set(
        day_stats[day_stats["clean_day"] == True][["hhid", "date"]].itertuples(
...
def calc_per_hhid(valid_hhid, df, cooking_events, final_output):
    ...
    final_consumption = (
        df_unique_fuel.groupby(["hhid", "date", "fuel_type"])["consumption (kg)"]
        .sum()
        .reset_index()
    )
